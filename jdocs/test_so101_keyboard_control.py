#!/usr/bin/env python3
"""
Simplified keyboard control for SO-101 robot
Fixed action format conversion issues
Uses P control, keyboard only changes target joint angles

MODIFIED FROM: XLeRobot/software/examples/0_so100_keyboard_joint_control.py
CHANGES FOR SO-101:
  - Line 277: Changed import from SO100Follower to SO101Follower
  - Line 281: Updated prompt text from SO100 to SO-101
  - Lines 291-292: Changed SO100FollowerConfig/SO100Follower to SO101FollowerConfig/SO101Follower
"""
...
def main():
    """Main function"""
    print("LeRobot SO-101 Keyboard Control Test (P Control)")
    print("="*50)

    try:
        # ========== CHANGE #1: Import SO101Follower instead of SO100Follower ==========
        from lerobot.robots.so101_follower import SO101Follower, SO101FollowerConfig
        from lerobot.teleoperators.keyboard import KeyboardTeleop, KeyboardTeleopConfig

        # ========== CHANGE #2: Updated prompt text from SO100 to SO-101 ==========
        port = input("Please enter SO-101 robot USB port (e.g.: /dev/ttyACM0): ").strip()
        ...
        # ========== CHANGE #3: Use SO101FollowerConfig and SO101Follower ==========
        # Added id parameter to use existing calibration file
        # change to left_arm when using left arm. Just enter for left arm but have to write /dev/ttyACM1 for right arm.
        robot_config = SO101FollowerConfig(port=port, id="xlerobot_right_arm")
