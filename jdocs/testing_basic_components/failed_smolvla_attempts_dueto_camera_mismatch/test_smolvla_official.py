#!/usr/bin/env python3
"""
Test SmolVLA with Official LeRobot API (Correct Version)
...
This uses the correct import paths and API for your LeRobot version.
"""
...
def test_smolvla_official(arm_port="/dev/ttyACM0", arm_id="xlerobot_left_arm",
    ...
    print("\n" + "="*60)
    print("SmolVLA Official API Test")
    print("="*60)
    ...
    try:
        ...
        # Step 2.5: Load dataset for normalization stats
        print("\nStep 2.5: Loading dataset for normalization stats...")
        print("⏳ First run: downloading dataset (~500MB-1GB, 2-5 minutes)")
        print("   Subsequent runs: loads from cache (fast)")
        try:
            dataset = LeRobotDataset("lerobot/svla_so101_pickplace")
            print("✅ Dataset loaded")
            print(f"   Dataset has stats: {dataset.meta.stats is not None}")
        ...
        # Step 3: Setup preprocessors/postprocessors
        print("\nStep 3: Setting up preprocessors...")
        try:
            # CRITICAL: Override the postprocessor's unnormalizer to use correct SO101 stats!
            # The base model has SO100 stats baked in, but we need SO101 stats
            ...
            print("   ✅ Postprocessor overridden with SO101 dataset stats!")
        except Exception as e:
        ...
        try:
            # CRITICAL: use_degrees=True because training data stats are in degrees!
            ...
            print(f"   ✅ Robot configured to accept DEGREES (matching training data)")
        except Exception as e:
...
def main():
    ...
    print("="*60)
    print("XLeRobot SmolVLA Test (Official API)")
    print("="*60)
    ...
    if success:
        ...
        print("\n2. Run evaluation (10 trials)")
        print("3. Proceed to Stage 3a (language control)")
    else:
        print("\n" + "="*60)
